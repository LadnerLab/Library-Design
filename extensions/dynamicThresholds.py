...
#    p.add_argument('--avgXmer_avgCol', default="Avg9mers", help='Header name in --avgXmer file corresponding to the avgXmer value of interest.')
    ...
    reqArgs.add_argument('-d', '--dir', help='Either a path to directory containing designs from SW_SC.py OR a path to a file containing multiple directory paths with designs.', required=True)
...
#        print("Opened metadata file for writing.")
    ...
    if os.path.isfile(args.dir):
        ...
        print("Cannot run. --dir must be either a file or a directory.")
        return False
    
    #Step through each directory with designs
    for directory in dirPaths:
        #Get fasta file paths
        fastaL = glob.glob("%s/*%s" % (directory, args.pepFasSuffix))
        fastaD = {os.path.basename(x).split("_SWSC")[0]:x for x in fastaL}

        #Get manifest file paths
        maniL = glob.glob("%s/*%s" % (directory, args.manifestSuffix))
        ...
        for k,v in fastaD.items():
            if k not in maniD:
                ...
                if not args.avgXmer:
                    try:
                        baseP = "/".join(v.split("/")[:-2])
                        clusName = os.path.basename(v).split("_SWSC")[0]
                        clusFile = f"{baseP}/{clusName}"
                        axm = avgTargetsPerKmer(clusFile, args.xmerSize, exSet=set("X-"))
                    except:
                        print(f"Failed to find {clusFile}, setting avgXmer value to 1.")
                        axm = 1
                else:
